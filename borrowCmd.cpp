...
void BorrowCmd::execute(MovieStore &store, const vector<string> &vs) const {
    ...
    if (genre == 'F') {
        ...
        if (type != 'D') {
            ...
            string str;
            getline(iss, str);
            cout << "Invalid media type " << type << ", discarding line: " << 
            str << ", " << vs[1] << endl;
            return;
        }
        ...
        if (customer == nullptr) {
            // Invalid Customer
            // output: Invalid customer ID <id>, discarding line:  D F You've Got Mail, 1998"
            istringstream iss(vs[0]);
            string str;
            iss >> cmd >> id;
            getline(iss, str);
            cout << "Invalid customer ID " << id << ", discarding line: " << 
            str << ", " << vs[1] << endl;
            return;
        }
    ...
    if (genre == 'C') {
        ...
        if (customer == nullptr) {
            // Invalid Customer
            // output: Invalid customer ID <id>, discarding line:  D C release date director
            istringstream iss(vs[0]);
            string str;
            iss >> cmd >> id;
            getline(iss, str);
